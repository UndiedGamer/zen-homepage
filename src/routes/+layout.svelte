<script lang="ts">
  ...
  onMount(() => {
    ...
    function draw() {
      ...
      let { bg, opacity, texture } = readZenThemeCSSVariables();

      // Fallbacks if the extension hasn't set the vars yet
      const defaultBg = 'linear-gradient(135deg, rgba(59,130,246,1) 0%, rgba(147,51,234,1) 100%)';
      const o = Number.isFinite(opacity) ? opacity : 1;
      const t = Number.isFinite(texture) ? texture : 0;

      const gradientStr = bg && bg.length ? bg : defaultBg;
      const colors = parseLinearGradientColors(gradientStr);

      // If somehow still no stops, bail with a solid fill
      if (!colors.length) {
      ...
      drawGradient(colors, o, ctx, canvas);
      drawTexture(t, ctx, canvas);
    }
